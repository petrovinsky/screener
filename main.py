    ...
    if not results:
        ...
        for _, row in df_res.iterrows():
            if 'Error' in row and row.get('Error'):
                table.add_row(row['Ticker'], "-", "-", "-", "-", "-", "-", "-", "-", f"[red]{row['Error']}[/red]")
                continue

            # Force string conversion & handle N/A
            earnings_val = str(row.get('Earnings', 'No'))
            analyst_val = str(row.get('Analyst', 'N/A'))
            signal_val = str(row.get('Signal', 'Neutral'))

            earn_color = "green" if "Yes" in earnings_val else "white"
            analyst_color = "green" if any(s in analyst_val.lower() for s in ['yes', 'strong', 'buy']) else \
                "red" if "no" in analyst_val.lower() else "yellow"
            signal_color = "green" if any(
                x in signal_val.lower() for x in ["bounce", "recovery", "bull", "hammer", "cross"]) else "yellow"
            ...
            table.add_row(
                ...
                str(row['MACD']),
                str(row['BB Pos']),
                color_text(earnings_val, earn_color),
                color_text(analyst_val, analyst_color),
                str(row['Bull Score']),
                color_text(signal_val, signal_color)
            )
